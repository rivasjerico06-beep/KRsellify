...
export default function Header() {
  ...
  return (
    <motion.header
      ...
      style={{ background: 'var(--white)', borderBottom: '2px solid var(--gray)', position: 'sticky', top: 0, zIndex: 1000, boxShadow: '0 2px 20px rgba(9,52,89,0.08)' }}
    >
      {/* top row */}
      <div style={{ maxWidth: 1340, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', gap: 16, height: 64 }}>
        <a href="/" style={{ fontFamily: 'var(--font-playfair)', fontSize: 24, fontWeight: 900, color: 'var(--navy)', letterSpacing: '-0.02em', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginLeft: 'auto', flexShrink: 0 }}>
            ...
            <AnimatePresence>
              {userMenuOpen && (
                <motion.div
                  ...
                  style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, background: 'white', border: '1px solid var(--gray)', borderRadius: 16, boxShadow: '0 8px 32px rgba(9,52,89,0.12)', minWidth: 200, overflow: 'hidden', zIndex: 100 }}
                  onMouseLeave={() => setUserMenuOpen(false)}
                >
                  {user ? (
                    <>
                      <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--gray)', background: 'var(--off-white)' }}>
                        <p style={{ fontWeight: 700, fontSize: 14, color: 'var(--navy)' }}>{profile?.full_name || user.email}</p>
                    ...
                    <>
                      <a href="/login" onClick={() => setUserMenuOpen(false)}
                        style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '13px 18px', fontSize: 14, color: 'var(--navy)', fontWeight: 700 }}
                        onMouseEnter={e => (e.currentTarget.style.background = 'var(--off-white)')}
                        onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
          ...
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setCartOpen(true)}
            ...
            <span className="kr-header-cart-label" style={{ fontWeight: 700, fontSize: 14, color: 'var(--navy)' }}>${cartTotal.toFixed(2)}</span>
          </motion.button>

          {/* hamburger */}
          <motion.button className="kr-mobile-menu-btn" whileTap={{ scale: 0.9 }}
            onClick={() => setMobileMenuOpen(o => !o)}
            style={{ display: 'none', background: 'none', border: 'none', cursor: 'pointer', fontSize: 20, color: 'var(--navy)', padding: 6, borderRadius: 8, alignItems: 'center', justifyContent: 'center' }}>
