    ...
    <InfoPageLayout title="Shipping Information" subtitle="Fast, tracked, and fully insured delivery worldwide." breadcrumb="Shipping">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <section style={{ background: 'white', borderRadius: 16, padding: '28px 32px', boxShadow: '0 2px 10px rgba(9,52,89,0.06)' }}>
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 20, fontWeight: 800, color: 'var(--navy)', marginBottom: 18 }}>Delivery Times</h2>
            ...
            <tbody>
              {[
                ...
                <tr key={region} style={{ borderBottom: '1px solid var(--gray)', background: i % 2 === 0 ? 'transparent' : 'rgba(244,248,248,0.5)' }}>
                  <td style={{ padding: '12px 14px', fontWeight: 600, color: 'var(--navy)' }}>{region}</td>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
          {[
            ...
            <div key={c.icon} style={{ background: 'white', borderRadius: 14, padding: '22px 20px', boxShadow: '0 2px 10px rgba(9,52,89,0.06)' }}>
              <i className={`fa-solid ${c.icon}`} style={{ color: 'var(--teal)', fontSize: 20, marginBottom: 12, display: 'block' }} />
              <h3 style={{ fontWeight: 700, fontSize: 14, color: 'var(--navy)', marginBottom: 6 }}>{c.title}</h3>
        ...
        <section style={{ background: 'white', borderRadius: 16, padding: '28px 32px', boxShadow: '0 2px 10px rgba(9,52,89,0.06)' }}>
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 20, fontWeight: 800, color: 'var(--navy)', marginBottom: 14 }}>Important Notes</h2>
