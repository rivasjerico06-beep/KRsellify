...
export async function POST(request: Request) {
  ...
  // Resolve user from Bearer token (optional — guest checkout not allowed from UI but keep graceful)
  ...
  // Auto-issue loyalty coupons if user is authenticated
  if (userId) {
    ...
    for (const { min, tier, pct, label } of TIERS) {
      if (totalSpent >= min) {
        // Check if they already have this tier coupon
        ...
        break // Only issue the highest tier reached
      }
    }
  }
